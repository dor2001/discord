...
# Healthcheck follows PORT (or PANEL_PORT fallback)
HEALTHCHECK --interval=30s --timeout=5s --start-period=20s --retries=3   CMD node -e "require('http').get('http://127.0.0.1:'+(process.env.PORT||process.env.PANEL_PORT||3000)+'/healthz',r=>process.exit(r.statusCode<500?0:1)).on('error',()=>process.exit(1))"
